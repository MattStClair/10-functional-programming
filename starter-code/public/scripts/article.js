...
(function(module){
  ...
  Article.numWordsAll = () => {
    return Article.all.map(obj => obj.body).reduce(function(acc, current){
      return acc + current.split(' ').length;
    },0)
  };
  ...
  Article.allAuthors = () => {
    return Article.all.map(author => author.author).reduce(function(acc, ele, pos, array){
      if(array.indexOf(ele)===pos){
        acc.push(ele);}
      return acc},[]);
  };
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(function(author){
      return {author: author, words: Article.all.filter(function(val){
        ...
        return acc + cur.body.split(' ').length
      },0)
      }})
